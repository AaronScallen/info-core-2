...
export default function BodycamsPage() {
  const { user } = useAuth();
  const [bodycams, setBodycams] = useState<Bodycam[]>([]);
  ...
  const fetchBodycams = async () => {
    try {
      setLoading(true);
      const response = await fetch("http://localhost:3001/api/bodycams", {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Are you sure you want to delete this bodycam?")) {
      return;
    }

    try {
      const response = await fetch(`http://localhost:3001/api/bodycams/${id}`, {
        method: "DELETE",
        headers: getAuthHeaders(),
      });

      if (!response.ok) {
        throw new Error("Failed to delete bodycam");
      }

      fetchBodycams();
    } catch (err) {
      alert(err instanceof Error ? err.message : "Failed to delete bodycam");
    }
  };

  const filteredBodycams = bodycams.filter((bodycam) => {
    ...
    return (
      ...
      bodycam.assignedToBadge?.toString().includes(searchLower)
    );
  });
            ...
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                  Manage body camera equipment
                </p>
              </div>
              <div className="flex items-center gap-4">
                <Link
                  href="/dashboard/bodycams/unassigned"
                        ...
                        >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div className="flex justify-end gap-2">
                              ...
                              {canDelete && (
                                <button
                                  onClick={() => handleDelete(bodycam.bwcId)}
